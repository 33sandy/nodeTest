...
module.exports = app =>{         //一个函数
    ...
    router.put('/',async (req,res)=>{
        if(req.Model.modelName == 'About'){
            let query = { /* query */ };
            req.body.expire=new Date();
            let update=req.body;
            let options = {upsert: true, new: true, setDefaultsOnInsert: true};
            // console.log(update)
            let model = await req.Model.findOneAndUpdate(query, update, options);
            console.log(model)
            }
        res.send({
            status:200,
    ...
    //挂载资源
    app.use('/admin/api/rest/:resource',authMiddleware(),resourceMiddleware(),router)        //挂载子路由，操作增删改查
